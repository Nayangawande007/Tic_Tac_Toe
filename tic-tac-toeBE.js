...
async function getInput(takeInput) {
  return await prompt.get([takeInput]);
}
...
//2)Converting box number to cordinate
...
// Function for turn ==> Optimization round

async function getCoordForTurn(gameGrid, playerNumber, isAgainstComputer) {
  if (playerNumber === 2 && isAgainstComputer) {
    return generateRandomCoords(gameGrid);
  }

  while (true) {
    let boxNumber = Object.values(
      await getInput(`Player ${playerNumber} turn\nPlease enter boxNumber \nGrid Box Number\n0 | 1 | 2\n3 | 4 | 5 \n6 | 7 | 8`)
    )[0];
...
async function playGame(gameGrid, isAgainstComputer = false) {
  for (let turn = 0; turn < 9; turn++) {
    const playerNumber = (turn % 2) + 1;
    const [xCoord, yCoord] = await getCoordForTurn(
      gameGrid,
      playerNumber,
      isAgainstComputer
    );
...
async function solve() {
  while (true) {
    let playWithComputer = Object.values(
      await getInput("Do you want to play with computer (Y/N)")
    )[0];
